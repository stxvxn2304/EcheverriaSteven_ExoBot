...
public class esAppException extends Exception {
    ...
    private void saveLogFile(String logMsg, Class<?> clase, String metodo) {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(esAppConfig.getLOGFILE(), true))) {
            System.err.println(esCMDColor.BLUE  + logMsg);
            writer.println(logMsg);
        } catch (Exception e) {
            System.err.println(esCMDColor.RED  + "[AppException.saveLogFile] ❱ " + e.getMessage());
        }finally {
