...
import esApp.esUserInterface.esCustomControl.PatButton;
import esApp.esUserInterface.esCustomControl.PatLabel;
import esApp.esUserInterface.esCustomControl.PatTextBox;
...
import esInfrastructureComponent.esAppMSG;
import esInfrastructureComponent.esAppStyle;
import esInfrastructureComponent.esTools.esCMD;
...
public class esPSexo extends JPanel implements ActionListener {
    ...
    public esPSexo(){
        try {
            ...
            esAppMSG.show("Error al cargar los datos tipo de persona: " + ex.getMessage());
        }
    }
    ...
    private void showRowData() {
        boolean isDTONull = (dtoSexo == null || dtoSexo.getEsIdSexo() == null);
        txtIdSexo.setText   ((isDTONull) ? " " : dtoSexo.getEsIdSexo().toString());
        txtNombre.setText   ((isDTONull) ? " " : dtoSexo.getEsNombre());
    ...
    private void showDataTable() throws Exception {
        ...
        for (esSexoDTO  d : blFactory.getAll()) {
            data[index][0] = d.getEsIdSexo();
            data[index][1] = d.getEsNombre();
        ...
        try {
            if (esAppMSG.showConfirmYesNo("¿Seguro que desea " + ((isDTONull) ? "AGREGAR ?" : "ACTUALIZAR ?"))) {
                if (isDTONull)  
                    ...
                    dtoSexo.setEsNombre(txtNombre.getText());
            ...
            esAppMSG.showError( ex.getMessage());
        }
    }
    private void btnEliminarClick() {
        try {
            if (esAppMSG.showConfirmYesNo("Seguro que desea Eliminar?")) {
                ...
                if (!blFactory.del(dtoSexo.getEsIdSexo()))
